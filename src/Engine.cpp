...
namespace brassica {
	...
	void Engine::DrawFrame() {
		...
		glm::vec3   sunRadiance =
			(lights.size() > 0) ? (lights[0].color * lights[0].intensity) : glm::vec3(3.0f, 2.94f, 2.76f);
		glm::vec3 moonDir = (lights.size() > 1) ? glm::normalize(-lights[1].direction) : glm::vec3(0.0f, -1.0f, 0.0f);
		glm::vec3 moonRadiance =
			(lights.size() > 1) ? (lights[1].color * lights[1].intensity) : glm::vec3(0.1f, 0.12f, 0.16f);
