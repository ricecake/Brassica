...
namespace brassica {
	...
	std::vector<Light>& LightManager::GetLights() { return _lights; }
	const std::vector<Light>& LightManager::GetLights() const { return _lights; }

	void LightManager::Update(float deltaTime) {
		if (_cycle.enabled) {
			if (!_cycle.paused) {
				_cycle.time += deltaTime * _cycle.speed;
				if (_cycle.time >= 24.0f) _cycle.time -= 24.0f;
				if (_cycle.time < 0.0f) _cycle.time += 24.0f;
			}
			...
			if (_lights.size() >= 2 && _lights[0].type == DIRECTIONAL_LIGHT && _lights[1].type == DIRECTIONAL_LIGHT) {
				...
				if (moonTime < 0.0f) moonTime += 24.0f;
		...
		for (auto it = _lights.begin(); it != _lights.end();) {
			...
			case LightBehaviorType::BLINK: {
				...
				light.intensity = (phase < light.behavior.period * light.behavior.dutyCycle) ? light.baseIntensity : 0.0f;
				break;
			}
			...
			case LightBehaviorType::FLICKER: {
				...
				if (light.behavior.flickerIntensity <= 0.5f) {
					...
					if (r < chance) {
						light.intensity = light.baseIntensity * (static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
					}
				}
				break;
			}
			case LightBehaviorType::MORSE: {
				...
				if (light.behavior.morseSequence.empty()) break;
