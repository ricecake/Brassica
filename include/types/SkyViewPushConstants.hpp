...
namespace brassica {
	...
	struct SkyViewPushConstants {
		...
		alignas(4) std::uint32_t outIndex{0};
		alignas(4) std::uint32_t transmittanceIndex{0};
		alignas(4) std::uint32_t multiScatteringIndex{0};
		alignas(4) std::uint32_t padding2{0};
	};

	static_assert(offsetof(SkyViewPushConstants, outIndex) == 80, "outIndex must be at offset 80");
	static_assert(sizeof(SkyViewPushConstants) == 96, "SkyViewPushConstants size must be 96 bytes");
