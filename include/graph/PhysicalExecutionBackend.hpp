...
namespace brassica::graph {
	...
	public:
		static bool Begin(
			...
			for (const auto& r : recipe.realizations) {
				...
				if (r.desc.width > renderWidth) renderWidth = r.desc.width;
				if (r.desc.height > renderHeight) renderHeight = r.desc.height;

				VkFormat format = static_cast<VkFormat>(tex->desc.formatCode);
				bool isDepth = (format == VK_FORMAT_D32_SFLOAT || format == VK_FORMAT_D24_UNORM_S8_UINT || format == VK_FORMAT_D16_UNORM);

				if (isDepth) {
					depthAttachment.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO;
					depthAttachment.imageView = tex->view;
					depthAttachment.imageLayout = VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
					depthAttachment.loadOp = (r.access == AccessKind::Write) ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_LOAD;
					...
					colorAtt.loadOp = (r.access == AccessKind::Write) ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_LOAD;
			...
			PFN_vkCmdBeginRendering pfnCmdBeginRendering =
				reinterpret_cast<PFN_vkCmdBeginRendering>(vkGetDeviceProcAddr(nullptr, "vkCmdBeginRendering"));
		...
		static void End(VkCommandBuffer cmd) {
			...
			PFN_vkCmdEndRendering pfnCmdEndRendering =
				reinterpret_cast<PFN_vkCmdEndRendering>(vkGetDeviceProcAddr(nullptr, "vkCmdEndRendering"));
	...
	public:
		...
		void Execute(Graph& graph, const FrameContext& ctx, CommandBuffer& cmd, bool enableAliasing = true) {
			...
			// 2. Compile schedule & barrier batches
			...
			// 5. Scheduled Node Execution Loop
			const auto& batches = graph.Batches();
			for (std::size_t slotIdx = 0; slotIdx < schedule.size(); ++slotIdx) {
				std::size_t nodeIndex = schedule[slotIdx];
				const auto& recipe = recipes[nodeIndex];

				// Dispatch Vulkan 1.3 barriers before node execution
				if (slotIdx < batches.size()) {
					BarrierTranslator::TranslateAndDispatch(vkCmd, m_registry, batches[slotIdx]);
				}

				// Begin Dynamic Rendering if graphics node
