	...
	public:
		...
		void Provision(
			...
			for (std::size_t slotIdx = 0; slotIdx < schedule.size(); ++slotIdx) {
				...
				if (nodeIndex >= recipes.size()) continue;
				...
				for (const auto& r : recipe.realizations) {
					...
					lt.firstPass = std::min(lt.firstPass, slotIdx);
					lt.lastPass = std::max(lt.lastPass, slotIdx);
				}
			}
			...
			for (std::size_t slotIdx = 0; slotIdx < schedule.size(); ++slotIdx) {
				...
				for (const auto& r : recipe.realizations) {
					if (m_textures.contains(r.key) && m_textures[r.key].isImported) continue;
					if (m_buffers.contains(r.key) && m_buffers[r.key].isImported) continue;
	...
	private:
		...
		void ProvisionTexture(ResourceId id, const ResourceDesc& desc, TemporalLifetime lifetime, bool enableAliasing) {
			...
			if (existingIt != m_textures.end() && !existingIt->second.isImported) {
				...
				if (existingIt->second.desc.width == desc.width &&
					existingIt->second.desc.height == desc.height &&
			...
			if (m_allocator && m_device) {
				...
				VkFormat format = static_cast<VkFormat>(desc.formatCode ? desc.formatCode : 44 /* VK_FORMAT_R8G8B8A8_UNORM */);
				VkImageUsageFlags usage = desc.usageMask ? static_cast<VkImageUsageFlags>(desc.usageMask)
				...
				imageInfo.extent = {desc.width ? desc.width : 1, desc.height ? desc.height : 1, desc.depth ? desc.depth : 1};
				...
				if (vmaCreateImage(m_allocator, &imageInfo, &allocInfo, &tex.image, &tex.allocation, nullptr) == VK_SUCCESS) {
		...
		void ProvisionBuffer(ResourceId id, const ResourceDesc& desc) {
			...
			if (existingIt != m_buffers.end() && !existingIt->second.isImported) {
				if (existingIt->second.desc.byteSize == desc.byteSize) return;
			}
