...
namespace brassica::graph {
	...
	DeriveImageState(AccessKind access, ExecutionDomain domain, vk::ImageUsageFlags usage, vk::Format format) {
		...
		if (domain == ExecutionDomain::Host) {
			if (isWrite) {
				...
				if (isReadWrite) acc |= vk::AccessFlagBits2::eHostRead;
				return ResourceState{vk::ImageLayout::eGeneral, vk::PipelineStageFlagBits2::eHost, acc};
			}
			return ResourceState{vk::ImageLayout::eGeneral, vk::PipelineStageFlagBits2::eHost, vk::AccessFlagBits2::eHostRead};
		}
	...
	inline ResourceState DeriveBufferState(AccessKind access, ExecutionDomain domain, vk::BufferUsageFlags usage) {
		...
		if (domain == ExecutionDomain::Transfer) {
			if (isWrite) {
				vk::AccessFlags2 acc = vk::AccessFlagBits2::eTransferWrite;
				if (isReadWrite) acc |= vk::AccessFlagBits2::eTransferRead;
