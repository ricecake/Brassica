...
namespace brassica::graph {
	...
	private:
		...
		static void EmitImageBarrier(
			...
			const std::uint32_t srcQueue = (srcQueueFamily != kQueueFamilyIgnored) ? srcQueueFamily : VK_QUEUE_FAMILY_IGNORED;
			const std::uint32_t dstQueue = (dstQueueFamily != kQueueFamilyIgnored) ? dstQueueFamily : VK_QUEUE_FAMILY_IGNORED;
