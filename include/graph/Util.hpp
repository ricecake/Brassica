...
namespace brassica::utils {
    ...
    std::shared_ptr<graph::PhysicalBuffer> CreateAndRegisterStaticBuffer(
        ...
        (void)transferQueue;
        registry.UploadPredefinedBuffer(
            graph::IdOf<Key>(),
            data.data(),
            data.size_bytes(),
            desc
	...
	inline void PhysicalResourceRegistry::UploadPredefinedTexture(
		...
		if (sizeBytes > 0 && allocator != VK_NULL_HANDLE && device && queue) {
			...
			vk::ImageSubresourceRange subresourceRange{
				vk::ImageAspectFlagBits::eColor, 0, texDesc.mips ? texDesc.mips : 1, 0, texDesc.layers ? texDesc.layers : 1
			};
			...
			cmd.copyBufferToImage(rawStagingBuffer, destTexture->GetImage(), vk::ImageLayout::eTransferDstOptimal, copyRegion);
