...
namespace brassica {
	...
	public:
		...
		Texture2D(
			...
			if (options.customUsageFlags) usageFlags = options.customUsageFlags;
			if (options.customMemoryUsage != VMA_MEMORY_USAGE_UNKNOWN) memoryUsage = options.customMemoryUsage;
			if (options.customAllocFlags) allocFlags = options.customAllocFlags;
		...
		void UploadPixelData(
			...
			vk::BufferCreateInfo bufferInfo({}, dataSize, vk::BufferUsageFlagBits::eTransferSrc, vk::SharingMode::eExclusive);
	...
	public:
		...
		GlobalDescriptorPool(vk::Device device, CapacityOptions opts = {}): m_device(device) {
			...
			vk::DescriptorPoolCreateInfo poolInfo(flags, opts.maxSets, static_cast<uint32_t>(poolSizes.size()), poolSizes.data());
	...
	public:
		...
		Texture2D* CreateDepthBuffer(const std::string& name, uint32_t width, uint32_t height, TextureOptions opts = {}) {
			auto it = m_textures.find(name);
		...
		Texture2D* CreateComputeShaderTarget(const std::string& name, uint32_t width, uint32_t height, TextureOptions opts = {}) {
			auto it = m_textures.find(name);
			if (it != m_textures.end()) return &it->second;
