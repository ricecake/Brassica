...
namespace brassica {
	...
	struct Light {
		...
		LightGPU ToGPU() const {
			...
			gpu.flags = 0;
			if (castsShadow) gpu.flags |= LIGHT_FLAG_CASTS_SHADOW;
			if (volumetricShadow) gpu.flags |= LIGHT_FLAG_VOLUMETRIC_SHADOW;
			if (cameraRelative) gpu.flags |= LIGHT_FLAG_CAMERA_RELATIVE;
			if (cloudEmissive) gpu.flags |= LIGHT_FLAG_CLOUD_EMISSIVE;
			return gpu;
		}
		...
		static Light CreateDirectional(float azimuth, float elevation, float intens, const glm::vec3& col, bool shadows = false) {
